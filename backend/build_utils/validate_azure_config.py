...
def validate_connection_string_format():
    ...
    if "AccountName=" not in connection_string or "AccountKey=" not in connection_string:
        print("❌ AZURE CONFIGURATION ERROR: Invalid connection string format")
        print("   Connection string must contain 'AccountName=' and 'AccountKey=' parameters")
        print("\n💡 EXPECTED FORMAT:")
        print("   DefaultEndpointsProtocol=https;AccountName=<name>;AccountKey=<key>;EndpointSuffix=core.windows.net")
        return False
    
    print("✅ Azure Storage connection string format is valid")
    return True
...
def main():
    ...
    for validation_name, validation_func in validations:
        ...
        try:
            ...
            print(f"❌ {validation_name} failed with exception: {str(e)}")
            failed_validations.append(validation_name)
