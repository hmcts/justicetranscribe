...
class BlobDeletionService:
    ...
    async def verify_transcription_completion(self, transcription_job_id: UUID) -> bool:
        ...
        try:
            async with get_async_session() as session:
                ...
                stmt = select(TranscriptionJob).where(TranscriptionJob.id == transcription_job_id)
                result = await session.execute(stmt)
                transcription_job = result.scalar_one_or_none()

                if not transcription_job:
                    logger.warning(f"Transcription job {transcription_job_id} not found")
                    return False

                # Check if transcription has dialogue entries (indicates successful completion)
                if not transcription_job.dialogue_entries:
                    logger.warning(f"Transcription job {transcription_job_id} has no dialogue entries")
                    return False

                # Check if there's an error message (indicates failure)
                if transcription_job.error_message:
                    logger.warning(f"Transcription job {transcription_job_id} has error: {transcription_job.error_message}")
                    return False

                logger.info(f"Transcription job {transcription_job_id} verified as completed successfully")
                return True

        except Exception as e:
            logger.error(f"Failed to verify transcription completion for {transcription_job_id}: {e}")
            return False
    ...
    async def delete_audio_blob(
        ...
        """
        logger.info(f"Starting blob deletion for {blob_path} (job: {transcription_job_id})")

        for attempt in range(1, self.max_retry_attempts + 1):
            try:
                # Check if blob exists before attempting deletion
                if not await self.azure_blob_manager.blob_exists(blob_path):
                    logger.warning(f"Blob {blob_path} does not exist, skipping deletion")
                ...
                if success:
                    logger.info(f"Successfully deleted blob {blob_path} on attempt {attempt}")
                    await self._mark_cleanup_complete(transcription_job_id)
                    return True
                else:
                    logger.warning(f"Failed to delete blob {blob_path} on attempt {attempt}")

            except Exception as e:
                logger.error(f"Exception during blob deletion attempt {attempt} for {blob_path}: {e}")

            # Wait before retry (except on last attempt)
            if attempt < self.max_retry_attempts:
                logger.info(f"Retrying blob deletion in {self.retry_delay_seconds} seconds...")
                await asyncio.sleep(self.retry_delay_seconds)

        # All retry attempts failed
        error_msg = f"Failed to delete blob {blob_path} after {self.max_retry_attempts} attempts"
        logger.error(error_msg)
        await self._mark_cleanup_failed(transcription_job_id, error_msg, user_email, blob_path)
        return False
    ...
    async def process_transcription_cleanup(
        ...
        """
        logger.info(f"Starting transcription cleanup process for job {transcription_job_id}")

        # Step 1: Verify transcription completion
        if not await self.verify_transcription_completion(transcription_job_id):
            logger.error(f"Transcription job {transcription_job_id} not completed successfully, skipping cleanup")
            return False

        # Step 2: Delete the audio blob
        deletion_success = await self.delete_audio_blob(blob_path, transcription_job_id, user_email)

        if deletion_success:
            logger.info(f"Transcription cleanup completed successfully for job {transcription_job_id}")
        else:
    ...
    async def get_jobs_needing_cleanup(self) -> list[TranscriptionJob]:
        ...
        try:
            async with get_async_session() as session:
                ...
                logger.info(f"Found {len(jobs)} transcription jobs needing manual cleanup")
                return list(jobs)
