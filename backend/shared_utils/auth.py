# ruff: noqa: TRY002, TRY003, TRY401, G004, B904, EM101, EM102, SIM103, SIM211
import logging
import os
...
def parse_auth_token(auth_header) -> tuple[str, list[str]]:
    ...
    verify_jwt_source = (
        False if os.environ.get("DISABLE_AUTH_SIGNATURE_VERIFICATION") else True
    )
...
def is_authorised_user(auth_header) -> bool:
    ...
    if os.environ.get("REPO") in roles:
        return True
    return False
