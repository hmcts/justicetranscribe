...
def create_allowlist(environment: str) -> Path:
    ...
    if environment == "dev":
        ...
        filtered_df = out_df.query("provider == 'ai justice unit'").reset_index(drop=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update allowlist files and upload to Azure")
    parser.add_argument(
        "--env", 
